...
export const SkillSelector = (props) => {
  ...
  const handleCreate = (inputValue) => {
    let newOption = { label: upperCase(inputValue)+" | "+selectedSkill, value: inputValue.toLowerCase()+"_"+selectedSkill };
  ...
  const saveProfile = async () => {
    ...
    let apiReq = await Axios.put(`http://localhost:5000/api/users/${userData.type}/${userData.user._id}`, {
      ...
      >
        <CreatableSelect
          ...
          formatCreateLabel={(inputValue) =>
            `Create Option:  ${upperCase(inputValue)} | ${selectedSkill}`
          }
